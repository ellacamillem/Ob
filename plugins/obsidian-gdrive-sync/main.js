  ...
  constructor() {
    ...
    this.refreshAll = async () => {
      ...
      try {
        ...
        var toDownload = this.cloudFiles.filter((file2) => !this.localFiles.includes(file2) && !this.renamingList.includes(file2) && !this.deletingList.includes(file2));
        await this.writeToVerboseLogFile("LOG: Deleting files in refreshAll");
        this.app.vault.getFiles().map(async (file2) => {
          if (!this.cloudFiles.includes(file2.path) && !this.renamingList.includes(file2.path) && file2.path != this.currentlyUploading) {
            if (file2.extension != "md") {
              if (/-synced\.*/.test(file2.path)) {
...
`));
    ...
    this.getLatestContent = async (file, forced = false) => {
      try {
        ...
        if (this.cloudFiles.includes(file == null ? void 0 : file.path) && !this.syncQueue.length) {
          ...
          if (forced == "forced" || isBinaryFile && parseInt(this.settings.autoRefreshBinaryFiles) || timeStamp && cloudDate.getTime() > new Date(timeStamp[0]).getTime() + 3e3) {
            var id;
            ...
            if (this.syncQueue.length || isBinaryFile || this.writingFile)
              return;
    ...
    this.uploadNewAttachment = async (e) => {
      ...
      try {
        ...
        var newFileName = e.path.replace(fileExtensionPattern, "-synced" + e.path.match(fileExtensionPattern)[0]);
        this.currentlyUploading = newFileName;
        await this.app.vault.rename(e, newFileName);
        this.cloudFiles.push(newFileName);
        let id = await uploadFile(this.settings.accessToken, newFileName, buffer, this.settings.vaultId);
        this.currentlyUploading = null;
        new import_obsidian2.Notice("Uploaded!");
        new import_obsidian2.Notice("Please make sure that all links to this attachment are updated with the new name: " + newFileName, 5e3);
        return id;
      } catch (err) {
        await this.notifyError();
        await this.checkForConnectivity();
    ...
    this.registerEvent(this.app.vault.on("create", async (e) => {
      ...
      try {
        if (!this.connectedToInternet) {
          ...
          if (e instanceof import_obsidian2.TFile && !/-synced\.*/.test(e.path)) {
            if (e.extension != "md") {
...
var syncSettings = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    if (!this.plugin.settings.vaultInit) {
      new import_obsidian2.Setting(containerEl).setName("Initialize vault").setDesc("Create vault and sync all files to Google Drive").addButton((button) => {
